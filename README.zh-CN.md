...
WebDAV 连接信息（`remoteBaseUrl`、`username`、`passwordEnv`、`password`、`remoteDir`）和 `backupRetention` 放在 `settings.webdav.json`；`installMissingPackages`、`extraSyncFiles`、`extraSyncDirs`、`excludeSyncPaths`、`excludeMcpServers`、`remoteDefaultModel` 放在 `settings.json` 的 `webdavsync` 字段中。`webdavsync` 不会上传，拉取时保留本机设置。额外同步路径和忽略路径均相对于 `~/.pi`；同步 agent 目录中的文件使用 `agent/` 前缀，直接同步 `~/` 下的文件使用 `home/` 前缀，例如 `agent/AGENTS.grok.md` 或 `home/.pi-lens/config.json`。`excludeSyncPaths` 中的路径不会上传，拉取时也会保留本机对应文件。`excludeMcpServers` 中的 MCP 不会上传，因此其他机器拉取后不会获得这些 MCP。`remoteDefaultModel` 仅修改上传副本中 `settings.json` 的默认模型，默认值为 `antigravity/gemini-3.8-flash`；追加 `:high` 等后缀时只为该模型设置 `modelThinkingLevels`，不影响其他模型，也不会修改本机配置。
...
在 `~/.pi/agent/settings.json` 中另行配置：
